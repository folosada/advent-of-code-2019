  ...
  commands.forEach(command => {
    ...
    for (let i = 0; i < distanceValue; i++) {
      ...
      if (trace[positionName] instanceof Array) {
        trace[positionName].push(wire);
      } else {
        trace[positionName] = [wire];
      }
    }
  });
}
...
const getDistance = () => {
  let minorDistance = 999999999;
  let distance = 0;
  let distanceX = 0;
  let distanceY = 0;
  crosses.forEach(cross => {
    let position = cross.split('|');
    position[0] = Number(position[0]);
    position[1] = Number(position[1]);

    if (position[0] < 0) {
      distanceX = centralPort[0] - position[0];
    } else {
      distanceX = position[0] - centralPort[0];
    }

    if (position[1] < 0) {
      distanceY = centralPort[1] - position[1];
    } else {
      distanceY = position[1] - centralPort[1];
    }
