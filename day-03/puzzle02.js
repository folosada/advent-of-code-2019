const centralPort = [1, 1];
const trace = {};
const crosses = [];
const runner = (commands, wire) => {
  let currentPosition = Object.assign({}, centralPort);
  let positionName = '';
  let steps = 0;
  commands.forEach(command => {
    commandCode = command[0];
    distanceValue = Number(command.substring(1));
    for (let i = 0; i < distanceValue; i++) {
      if (commandCode === 'R') {
        currentPosition[0]++;
      }
      if (commandCode === 'L') {
      ...
      steps++;
      positionName = `${currentPosition[0]}|${currentPosition[1]}`;
      if (trace[positionName] instanceof Array) {
        const found = trace[positionName].find(item => item.wire === wire);
        if (!found) {
          trace[positionName].push({wire, steps});
        }
      } else {
        trace[positionName] = [{wire, steps}];
      }
    }
  });
}
...
const readTrace = () => {
  for (let position in trace) {
    tracePosition = trace[position];
    let unique = tracePosition.filter((item, i, ar) => ar.indexOf(item) === i);
    if (unique.length > 1) {
